...
def generate_response_based_on_course_details(user_message, product_details):
    ...
    Step 1: {delimiter} {product_details} into a correct JSON format.
    
    Step 2:{delimiter} If the user is asking about course, \
    understand the relevant course(s) from the following list.
    All available courses shown in the json data below:
    {product_details}

    Step 3:{delimiter} Use the information about the course to \
    ...
    If {product_details} are not available, lead your respond with "Unfortunately".

    Use the following format:
    Step 1:{delimiter} <step 1 instructions> {delimiter}
    Step 2:{delimiter} <step 2 reasoning>
    Step 3:{delimiter} <step 3 reasoning>
    Step 4:{delimiter} <step 4 response to customer>
    ...
    response_to_customer = llm.get_completion_by_messages(messages).split(delimiter)
    finalResponse = list(filter(None, response_to_customer))
